...
def softmax(x):
  ...
  ### YOUR CODE HERE
  shift_scores = x - tf.reduce_max(x, reduction_indices=[1], keep_dims=True)
  out = tf.exp(shift_scores)/tf.reduce_sum(tf.exp(shift_scores), reduction_indices=[1], keep_dims=True)
  ### END YOUR CODE
...
def cross_entropy_loss(y, yhat):
  ...
  ### YOUR CODE HERE
  y = tf.to_float(y) 
  out = -tf.reduce_sume(y * tf.log(yhat))
  ### END YOUR CODE
  return out
